...
def partial_load(pretrained_dict, model, skip_keys=[]):
    ...
    print(f'\nLoading keys: [{len(filtered_dict.keys())}/{len(model_dict.keys())}]\n', filtered_dict.keys())
...
def make_encoder(args):
    ...
    if model_type == 'crw':
        ...
        net_state = {k.replace('module.encoder.', ''):v for k,v in net_ckpt['state_dict'].items() \
                if 'module.encoder' in k}
        print(f"{net_ckpt['state_dict'].keys()},\n{len(net_ckpt['state_dict'].keys())}  \n\n{net_state.keys()},\n{len(net_state.keys())}\n\n")
    ...
    elif model_type == 'dino_r50':
        net = resnet.resnet50(pretrained=False)
        net_ckpt = torch.load(args.resume)
        net_state = {k:v for k,v in net_ckpt.items()}
        partial_load(net_state, net)
        
    elif model_type == 'vfs':
        net = resnet.resnet50(pretrained=False)
        net_ckpt = torch.load(args.resume)
        net_state = {k:v for k,v in net_ckpt['state_dict'].items()}
        partial_load(net_state, net)
