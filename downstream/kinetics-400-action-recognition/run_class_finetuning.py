...
import modeling_finetune_2d_vit
...
def get_args():
    ...
    # new args
    parser.add_argument('--patchify_type', default="3d", type=str)
    ...
    parser.add_argument('--gpus', default="2,3", type=str)
    ...
    os.environ['MASTER_PORT'] = '18979'  # 0号机器的可用端口，随便选一个没被占用的
    os.environ['CUDA_VISIBLE_DEVICES'] = known_args.gpus  # 使用哪些 GPU
...
def main(rank, args, ds_init):
    ...
    if args.patchify_type == "3d":
        print("#### load 3d_patch videoMAE ViT\n")
        time.sleep(2)
        model = create_model(
            args.model,
            pretrained=False,
    ...
    elif args.patchify_type == "2d":
        print("#### load 2d_patch ViT\n")
        time.sleep(2)
        model = modeling_finetune_2d_vit.__dict__[args.model](
            num_classes=args.nb_classes,
            drop_path_rate=args.drop_path,
            global_pool=True,
            )
    else:
        assert False, "error patchify_type"
    ...
    if args.patchify_type == "3d":
        ...
        pretrain_sd = torch.load(args.finetune)

        pretrain_sd["patch_embed.weight"] = utils.conv2d_weight_to_3d(pretrain_sd["patch_embed.proj.weight"], time_dim=2)
        pretrain_sd["patch_embed.bias"] = pretrain_sd["patch_embed.proj.bias"]

        missing_params = ""
        for i in model_sd.keys():
            if i not in pretrain_sd:
                missing_params+= (i + "\n")
            
        match_sd = {k:v for k,v in pretrain_sd.items() if k in model_sd}
        model_sd.update(match_sd)
        print(f"######### load [{len(match_sd)}/{len(model_sd)}] params\n######### model unload \n{missing_params}")
        time.sleep(10)
...
if __name__ == '__main__':
    try:
        ...
        mp.spawn(main, args=(opts, ds_init, ), nprocs=torch.cuda.device_count())
